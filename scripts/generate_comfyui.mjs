...
// ComfyUI 服务器配置
const COMFYUI_HOST_VM = '192.168.64.1:7860';  // 宿主机 Apple Container 网关
const COMFYUI_INTERNAL = '192.168.31.8:7860';    // 局域网 ComfyUI 服务器
const COMFYUI_EXTERNAL = '123.56.194.98:7860';  // 外网服务器

// 默认使用局域网服务器
const COMFYUI_DEFAULT = 'internal';
...
async function getComfyUIserver() {
  ...
  const mode = process.env.COMFYUI_MODE || COMFYUI_DEFAULT || 'internal';

  if (mode === 'external') {
    log(`使用外网服务器: ${COMFYUI_EXTERNAL}`);
    return COMFYUI_EXTERNAL;
  } else if (mode === 'internal') {
    log(`使用内网服务器: ${COMFYUI_INTERNAL}`);
    return COMFYUI_INTERNAL;
  } else {
    // Default: try host_vm first, fall back to external if unreachable
    log(`使用宿主机网关: ${COMFYUI_HOST_VM}`);
    const reachable = await testConnection(COMFYUI_HOST_VM);
    if (reachable) {
      return COMFYUI_HOST_VM;
    }
    log(`宿主机网关不可达，切换到外网服务器: ${COMFYUI_EXTERNAL}`);
    return COMFYUI_EXTERNAL;
  }
}
...
async function main() {
  ...
  if (args.length < 1) {
    ...
    console.error('  COMFYUI_MODE=internal  使用内网服务器 (192.168.31.8:7860)');
    console.error('  COMFYUI_MODE=external  使用外网服务器 (123.56.194.98:7860)');
    console.error('  COMFYUI_SERVER=地址    直接指定服务器地址');
    process.exit(1);
  }
